...
func main() {
    if len(os.Args) != 3 {
        errParametros := new(errores.ErrorParametros)
        fmt.Println(errParametros.Error())
        return
    }

	padrones, errdni := os.Open(os.Args[2])
	partidos, errPartidos := os.Open(os.Args[1])
	...
	if contadorImpugnados == 1 {
		fmt.Println("\nVotos Impugnados:", contadorImpugnados, "voto")
	} else {
		fmt.Println("\nVotos Impugnados:", contadorImpugnados, "votos")
	}
